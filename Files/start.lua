...
local reply_markup = bot.replyMarkup{
type = 'inline',data = {
{{text = "أوامر الحمايه" ,data="Amr_"..data.sender_user_id.."_1"},{text = "إعدادات المجموعه",data="Amr_"..data.sender_user_id.."_2"}},
...
{{text = ' ✦︙ تغيير رساله ستارت',data="addstarttxt"},{text =" ✦︙ حذف رساله ستارت",data="Deltxtstart"}},
...
bot.editMessageText(chat_id,msg_id,"*  ✦︙اهلا بك في قائمه الاوامر : العللامه ( ✅ ) تعني الامر مفعل و ( ❌ ) العكس*", 'md', true, false, reply_dev)
end
...
bot.editMessageText(chat_id,msg_id,"*  ✦︙قم الان بأرسل رساله ستارت الجديده*", 'md', true, false, reply_markup)
...
bot.answerCallbackQuery(data.id, "- تم حذف رساله ستارت بنجاح .", true)
end
...
bot.sendText(msg.chat_id,msg.id,"*  ✦︙اهلا بك في قائمه الاوامر : العللامه ( ✅ ) تعني الامر مفعل و ( ❌ ) العكس*","md", true, false, false, false, reply_dev)
end
end
...
if text == "/start" then 
bot.sendText(msg.chat_id,msg.id,"*  ✦︙اهلا بك في قائمه الاوامر : العللامه ( ✅ ) تعني الامر مفعل و ( ❌ ) العكس*","md", true, false, false, false, bot.replyMarkup{
type = 'inline',data = {
	{{text = ' ✦︙ الاحصائيات',data="indfo"}},
	{{text = ' ✦︙ تغيير المطور الاساسي',data="EditDevbot"}},
{{text = ' ✦︙ تغيير اسم البوت',data="namebot"},{text =(redis:get(bot_id..":namebot") or "يوسف"),data="delnamebot"}},
...
local Info = bot.searchChatMembers(msg.chat_id, "*", 200)
local members = Info.members
ls = "*  ✦︙قائمه البوتات في المجموعه\n  *ٴ─━─━─━─×─━─━─━─ *\n  ✦︙العللامه 《 *★ * 》 تدل على ان البوت مشرف*\n *ٴ─━─━─━─×─━─━─━─ *\n"
i = 0
for k, v in pairs(members) do
UserInfo = bot.getUser(v.member_id.user_id) 
if UserInfo.type.luatele == "userTypeBot" then 
...
reply_markup = bot.replyMarkup{
type = 'inline',data = {
{{text = "أوامر الحمايه" ,data="Amr_"..msg.sender.user_id.."_1"},{text = "إعدادات المجموعه",data="Amr_"..msg.sender.user_id.."_2"}},
...
srt = "messageVideo"
elseif TextMsg == 'الفارسيه' or TextMsg == 'الفارسيه بالكتم' or TextMsg == 'الفارسيه بالطرد' or TextMsg == 'الفارسيه بالتقييد' or TextMsg == 'الفارسيه بالتقييد' then
srt = "WordsPersian"
elseif TextMsg == 'الانجليزيه' or TextMsg == 'الانجليزيه بالكتم' or TextMsg == 'الانجليزيه بالطرد' or TextMsg == 'الانجليزيه بالتقييد' or TextMsg == 'الانجليزيه بالتقييد' then
srt = "WordsEnglish"
...
srt = "JoinByLink"
elseif TextMsg == 'الاضافه' or TextMsg == 'الاضافه بالكتم' or TextMsg == 'الاضافه بالطرد' or TextMsg == 'الاضافه بالتقييد' or TextMsg == 'الاضافه بالتقييد' then
srt = "AddMempar"
...
elseif TextMsg == 'القناه' or TextMsg == 'القناه بالكتم' or TextMsg == 'القناه بالطرد' or TextMsg == 'القناه بالتقييد' or TextMsg == 'القناه بالتقييد' then
srt = "messageSenderChat"
elseif TextMsg == 'الشارحه' or TextMsg == 'الشارحه بالكتم' or TextMsg == 'الشارحه بالطرد' or TextMsg == 'الشارحه بالتقييد' or TextMsg == 'الشارحه بالتقييد' then
srt = "Cmd"
...
srt = "messageVideo "
elseif TextMsg == 'الفارسيه' or TextMsg == 'الفارسيه' or TextMsg == 'الفارسي' then
srt = "WordsPersian"
elseif TextMsg == 'الانجليزيه' or TextMsg == 'الانجليزيه' or TextMsg == 'الانجليزي' then
srt = "WordsEnglish"
...
if text == 'الالعاب' or text == 'قائمه الالعاب' or text == 'قائمه الالعاب' then
...
if text == 'المجموعه' or text == 'عدد القورب' or text == 'عدد المجموعه' then
Get_Chat = bot.getChat(msg.chat_id)
Info_Chats = bot.getSupergroupFullInfo(msg.chat_id)
local reply_markup = bot.replyMarkup{
type = 'inline',
...
{{text="لعبه فلابي بيرد 🐥",url='https://t.me/awesomebot?game=FlappyBird'},{text="تحداني فالرياضيات 🔢",url='https://t.me/gamebot?game=MathBattle'}},
{{text="تحداني في ❌⭕️",url='t.me/XO_AABOT?start3836619'},{text="سباق الدراجات 🏍",url='https://t.me/gamee?game=MotoFX'}},
{{text="سباق سيارات 🏎",url='https://t.me/gamee?game=F1Racer'},{text="متشابه 👾",url='https://t.me/gamee?game=DiamondRows'}},
{{text="كره قدم ⚽",url='https://t.me/gamee?game=FootballStar'}},
...
{{text="لعبه 2048 🎰",url='https://t.me/awesomebot?game=g2048'},{text="Squares🏁",url='https://t.me/gamee?game=Squares'}},
...
local arr = {'آخر مره زرت مدينه الملاهي؟','آخر مره أكلت أكلتك المفضّله؟','الوضع الحالي؟\n‏1. سهران\n‏2. ضايج\n‏3. أتأمل','آخر شيء ضاع منك؟','كلمه أخيره لشاغل البال؟','طريقتك المعتاده في التخلّص من الطاقه السلبيه؟','شهر من أشهر العام له ذكرى جميله معك؟','كلمه غريبه من لهجتك ومعناها؟🤓','‏- شيء سمعته عالق في ذهنك هاليومين؟','متى تكره الشخص الذي أمامك حتى لو كنت مِن أشد معجبينه؟','‏- أبرز صفه حسنه في صديقك المقرب؟','هل تشعر أن هنالك مَن يُحبك؟','اذا اكتشفت أن أعز أصدقائك يضمر لك السوء، موقفك الصريح؟','أجمل شيء حصل معك خلال هاليوم؟','صِف شعورك وأنت تُحب شخص يُحب غيرك؟👀💔','كلمه لشخص غالي اشتقت إليه؟💕','آخر خبر سعيد، متى وصلك؟','أنا آسف على ....؟','أوصف نفسك بكلمه؟','صريح، مشتاق؟','‏- صريح، هل سبق وخذلت أحدهم ولو عن غير قصد؟','‏- ماذا ستختار من الكلمات لتعبر لنا عن حياتك التي عشتها الى الآن؟💭','‏- فنان/ه تود لو يدعوكَ على مائده عشاء؟😁❤','‏- تخيّل شيء قد يحدث في المستقبل؟','‏- للشباب | آخر مره وصلك غزل من فتاه؟🌚','شخص أو صاحب عوضك ونساك مُر الحياه ما اسمه ؟','| اذا شفت حد واعجبك وعندك الجرأه انك تروح وتتعرف عليه ، مقدمه الحديث ش
','/','أفضل صفه تحبه بنفسك؟','اكثر وقت تحب تنام فيه ...','أطول مده نمت فيها كم ساعه؟','أصعب قرار ممكن تتخذه ؟','أفضل صفه تحبه بنفسك؟','اكثر وقت تحب تنام فيه ...','أنت محبوب بين الناس؟ ولاكريه؟','إحساسك في هاللحظه؟','اخر شيء اكلته ؟','تشوف الغيره انانيه او حب؟','اذكر موقف ماتنساه بعمرك؟','اكثر مشاكلك بسبب ؟','اول ماتصحى من النوم مين تكلمه؟','آخر مره ضحكت من كل قلبك؟','لو الجنسيه حسب ملامحك وش بتكون جنسيتك؟','اكثر شيء يرفع ضغطك','اذكر موقف ماتنساه بعمرك؟','لو قالوا لك  تناول صنف واحد فقط من الطعام لمده شهر .','كيف تشوف الجيل ذا؟','رده فعلك لو مزح معك شخص م تعرفه ؟','احقر الناس هو من ...','تحب ابوك ولا امك','آخر فيلم مسلسل والتقييم🎥؟','أقبح القبحين في العلاقه: الغدر أو الإهمال🤷🏼؟','كلمه لأقرب شخص لقلبك🤍؟','حط@منشن لشخص وقوله "حركتك مالها داعي"😼!','اذا جاك خبر مفرح اول واحد تعلمه فيه مين💃🏽؟','طبع يمكن يخليك تكره شخص حتى لو كنتتُحبه🙅🏻‍♀️؟','افضل ايام الاسبوع عندك🔖؟','يقولون ان الحياه دروس ، ماهو أقوى درس تعلمته من الحياه🏙؟','تاريخ لن تنساه📅؟','تحب الصيف والا الشتاء❄️☀️؟','شخص تحب تستفزه😈؟','شنو ينادونك وانت صغير (عيارتك)👼🏻؟','عقل يفهمك/ج ولا قلب يحبك/ج❤️؟','اول سفره لك وين رح تكون✈️؟','كم عدد اللي معطيهم بلوك👹؟','نوعيه من الأشخاص تتجنبهم في حياتك❌؟','شاركنا صوره او فيديو من تصويرك؟📸','كم من عشره تعطي حظك📩؟','اكثر برنامج تواصل اجتماعي تحبه😎؟','من اي دوله انت🌍؟','اكثر دوله ودك تسافر لها🏞؟','مقوله "نكبر وننسى" هل تؤمن بصحتها🧓🏼؟','تعتقد فيه أحد يراقبك👩🏼‍💻؟','لو بيدك تغير الزمن ، تقدمه ولا ترجعه🕰؟','مشروبك المفضل🍹؟','‏قم بلصق آخر اقتباس نسخته؟💭','كم وزنك/ج طولك/ج؟🌚','كم كان عمرك/ج قبل ٨ سنين😈؟','دوله ندمت انك سافرت لها😁؟','لو قالو لك ٣ أمنيات راح تتحقق عالسريع شنو تكون🧞‍♀️؟','‏- نسبه احتياجك للعزله من 10📊؟','شخص تحبه حظرك بدون سبب واضح، رده فعلك🧐؟','مبدأ في الحياه تعتمد عليه دائما🕯؟'}
...
bot.sendText(msg.chat_id,msg.id,"*  ✦︙تم تغير رساله الايدي*","md",true)  
end
...
if BasicConstructor(msg) then
----------------------------------------------------------------------------------------------------
if text == "قائمه المنع" or text == "الممنوعات" or text == "قائمه المنع" then
...
return bot.sendText(data.message.chat_id,data.message.id,Reply_Status(data.message.sender.user_id,"*  ✦︙عليك الاشتراك في قناه البوت اولاً !*").yu,"md", true, false, false, false, reply_dev)
end
end
end
end
